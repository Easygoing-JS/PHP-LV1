    ...
    if ($_SERVER['REQUEST_METHOD'] != 'POST') :  // if: 1 SERVER['REQUEST_METHOD'] != 'POST'
        ...
        <main>
                ...
                if (count($mes) > 0) :  // if 2 контроль ошибок ввода
                    ...
                    <div align=center>
                        <form method=POST ACTION="mod_products_show.php" ENCTYPE="multipart/form-data">
                            ...
                            <input type=hidden name=id_redact
                                   value='<?php if (isset($_POST['id_redact']) && $_POST['id_redact'] > 0)
                                       echo $_POST['id_redact']; ?>'>

                            <input type=hidden name=price value='<?php if (isset($_POST['price']))
                                echo (int)$_POST['price']; ?>'>

                            <input type=hidden name=product_name value='<?php if (isset($_POST['product_name']))
                                echo htmlspecialchars(strip_tags(str_replace("'", "&#39;", trim($_POST['product_name'])))); ?>'>
                            <input type=hidden name=product_text value='<?php if (isset($_POST['product_text']))
                                echo htmlspecialchars(strip_tags(str_replace("'", "&#39;", trim($_POST['product_text'])))); ?>'>
            ...
            REPLACE INTO products (
                    ...
                    if ($id_redact_bd == '') {
                        $sql = "SELECT * FROM products WHERE  product_time='" . $product_time_bd . "' AND product_name=' . $product_name_bd . ';";
                    ...
                    if ($id_redact_bd > 0) {
                        ...
                        $sql = "DELETE  FROM uid_groups WHERE id_product=" . $id_redact_bd;
                        //echo $sql . "<br>";
                        $err = '';
